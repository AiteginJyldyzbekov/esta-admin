...
const UseDesignService = () => {
  ...
  const getServices = useCallback(async () => {
    const arr = [];
    const data = await getDocs(collection(db, "designServices"));
    data.forEach((doc) => {
  ...
  const addService = async (data) => {
    const res = await addDoc(collection(db, "designServices"), {
      ...data,
  ...
  const getServiceDetail = async (id) => {
    const docRef = doc(db, "designServices", id);
  ...
  const updateService = async (id, data) => {
    const ref = doc(db, "designServices", id);
