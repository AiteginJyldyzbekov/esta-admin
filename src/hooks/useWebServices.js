...
const UseWebService = () => {
  ...
  const getServices = useCallback(async () => {
    const arr = [];
    const data = await getDocs(collection(db, "webServices"));
    data.forEach((doc) => {
  ...
  const addService = async (data) => {
    const res = await addDoc(collection(db, "webServices"), {
      ...data,
  ...
  const getServiceDetail = async (id) => {
    const docRef = doc(db, "webServices", id);
  ...
  const updateService = async (id, data) => {
    const ref = doc(db, "webServices", id);
