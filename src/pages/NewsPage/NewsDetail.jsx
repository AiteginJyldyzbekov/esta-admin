...
function NewsDetail() {
    const { updateNews, getNewsDetail, newsDetail, isLoading, error } = useNews();
    const { id } = useParams();
    ...
    useEffect(() => {
        if (newsDetail) {
            ...
            setDesc1(newsDetail.desc1)
            setDesc2(newsDetail.desc2)
            setDate(new Date(newsDetail.date.split('/').reverse().join('-')))
            setImageData(newsDetail.images)
        }
    }, [newsDetail])
    ...
    const submit = (e) => {
        ...
        setSending(true);
        // console.log(dayjs(date).format("D/MM/YYYY"))
        updateNews(id, {
            title,
            desc1,
            desc2,
            date: dayjs(validDate).format("D/MM/YYYY"),
